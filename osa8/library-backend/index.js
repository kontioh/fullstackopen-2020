...
const resolvers = {
  Query: {
    ...
    allBooks: (root, args) => {
      ...
      return books.filter(b => b.author === args.author && b.genres.find(g => g === args.genre))
    },
    allAuthors: () => {
      let returnValues = []
  ...
  Mutation: {
    addBook: (root, args) => {
      if (!authors.find(a => a.name === args.author)) {
        const newAuthor = { name: args.author, born: null, id: uuid(), bookCount: 1 }
    ...
    editAuthor: (root, args) => {
      ...
      authors = authors.map(a => a.name !== args.name ? a : modifiedAuthor)
